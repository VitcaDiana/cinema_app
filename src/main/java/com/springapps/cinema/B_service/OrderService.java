...
public class OrderService {
    ...
    @Autowired
    public OrderService(OrderRepository orderRepository, UserRepository userRepository, PdfGenerationService pdfGenerationService, ProjectionRepository projectionRepository, SeatRepository seatRepository, SeatService seatService, TicketService ticketService, EmailService emailService) {
    ...
    public Order addOrder (OrderRequestDTO orderRequestDTO) throws MessagingException {
        ...
        pdfGenerationService.generatePdf("Ai cumparat bilet la filmul "+ projection.getMovie().getTitle(), "src/main/resources/order2.pdf");
        emailService.sendMessageWithAttachment(user.getName(),"sdfdsf", "dsfdsf", "src/main/resources/order2.pdf");
        return savedOrder;
